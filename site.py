...
def update_graph(gender, attributes, num_clusters):
    ...
    fig = px.scatter(pca_df, x=cluster_df.columns[0], y=cluster_df.columns[1], color='Cluster',
                     color_discrete_sequence=px.colors.qualitative.Set1, hover_name=hover_name, hover_data=hover_data, title=title)
    fig.update_layout(title_x=0.5, xaxis_title=cluster_df.columns[0], yaxis_title=cluster_df.columns[1])
